---
name: scrape
description: Finds new job postings matching your profile via installed portal-search CLIs (LinkedIn, local job boards, and any skills added with $add-portal). Deduplicates across runs. Trigger on "find new jobs", "scrape for jobs", "search jobs", "any new positions", or $scrape.
---
...
`.agents/skills/` (plus a web search as a fallback), using queries from your profile.
...
Optional arguments, given as the text following this skill invocation:
...
Read `.claude/skills/job-scraper/search-queries.md` for the search strategy. By default, run the top 3 priority query categories. If the user said "broad", run all categories. If the user specified a focus area (e.g. "data science"), prioritize queries from that category.

**Use the installed CLI tools as the primary search mechanism.** Fall back to a web search only for portals that do not have a CLI skill, or if `bun` is unavailable on the system.
...
Discover all installed portal CLI skills by scanning for every `SKILL.md` found under `.agents/skills/*/SKILL.md`. Each file documents that portal's exact CLI flags and usage examples. **Use each portal's own documented interface — do not guess flags.** This approach automatically includes any new portals added via `$add-portal` without requiring changes to this file.
...
Run all portal CLI calls in parallel where possible: delegate to a subagent per portal - ask Codex to spawn one subagent per installed portal CLI (running concurrently), each executing that portal's search command and returning its parsed results, and wait for all of them to finish before continuing. Collect all `results` arrays into a single pool for Step 2, keeping each result tagged with its source portal skill (for Step 2 `detail` lookups).
...
**From web search results:** Fetch the posting URL and extract the same
fields manually.
...
1. **Never fabricate job postings.** Only present jobs from actual CLI search/detail output or web search/fetch results.
2. **Respect deduplication.** Always check seen_jobs.json AND job_search_tracker.csv before presenting.
3. **Focus on configured geographic area.** Skip jobs that require relocation or are clearly outside commute range.
4. **Only open positions.** Skip postings with expired deadlines or those marked as closed.
5. **Be efficient with detail fetches.** Don't run `detail` or a page fetch on every search hit — pre-filter by title/snippet, then fetch only promising matches.
6. **Parallel searches.** Run portal CLI searches in parallel; use a web search only for gaps the CLIs don't cover.
