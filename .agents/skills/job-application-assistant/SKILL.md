...
description: Assists with job applications - evaluating job postings, tailoring CVs, writing cover letters, and preparing for interviews. Trigger on keywords like job posting, job application, CV, cover letter, resume, interview prep, job fit, career, application, apply, ansøgning, stilling, or $job-application-assistant.
---
...
- Fetch the job posting content (for URLs)
...
All seven reference files live under `.claude/skills/job-application-assistant/` (e.g. `.claude/skills/job-application-assistant/01-candidate-profile.md`) and are shared with the `apply` skill and the other ported skills - read them from that path.
...
For a more structured version of the full evaluate-draft-review-compile workflow, use the `$apply` skill instead - it adds a reviewer subagent critique pass and a mandatory PDF compile-and-inspect step that this quick-access skill does not.
